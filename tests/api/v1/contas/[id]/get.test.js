...
describe("GET /api/v1/contas/[id]", () => {
  describe("Usuario autenticado", () => {
    ...
    test("Com id inexistente", async () => {
      ...
      const session = await orchestrator.createSession(user.id);
      const response = await fetch(`http://localhost:3000/api/v1/contas/132`, {
        method: "GET",
        headers: {
          "Content-type": "application/json",
          Cookie: `sid=${session.token}`,
  ...
  describe("Usuario autenticado, sem permissao", () => {
    test("Com dados obrigatórios válidos", async () => {
      ...
      const session = await orchestrator.createSession(user.id);
      const response = await fetch("http://localhost:3000/api/v1/contas/1", {
        method: "GET",
        headers: {
          "Content-type": "application/json",
          Cookie: `sid=${session.token}`,
